/* app.jsx — Qurban Alfatihah landing page assembly + Tweaks */
...
function App(){
  ...
  /* enable hidden-until-revealed states only once JS is confirmed running */
  useEffect(()=>{ document.documentElement.classList.add('js-reveal'); },[]);

  /* force a reflow once the webfont loads so wrapped headings get correct box heights */
  useEffect(()=>{
  ...
  /* animate program progress bars when visible */
  useEffect(()=>{
    const cleanups=[];
    document.querySelectorAll('.prog-progress i[data-w]').forEach(b=>{
      cleanups.push( window.onInView(b, ()=>{ b.style.width = b.dataset.w+'%'; }) );
    });
    return ()=>cleanups.forEach(c=>c());
  },[t.progLayout]);

  const openOrder=(preset=null)=> setModal({ open:true, preset: typeof preset==='string'?preset:null });
  const closeOrder=(submitted)=>{
    setModal(m=>({...m,open:false}));
    if(submitted===true){ setToast(true); setTimeout(()=>setToast(false),3600); }
  };
  ...
  return (
    <React.Fragment>
      ...
      <OrderModal open={modal.open} preset={modal.preset} onClose={closeOrder}/>
